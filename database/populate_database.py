...
SQL_FILE = os.path.join(DB_DIR, 'create_database.sql')
...
def populate_time_slots(conn):
    """Populate standard time slots"""
    slots = []
    
    # M/W/F slots (Daily for 1 hour)
    mwf_times = [
        ("08:00", "09:00"),
        ("09:00", "10:00"),
        ("10:00", "11:00"),
    ...
    for start, end in mwf_times:
        for day in ["Monday", "Wednesday", "Friday"]:
            slots.append((day, start, end))

    # T/Th slots (Longer, 1.5 hours)
    tth_times = [
        ("08:00", "09:30"),
        ("09:30", "11:00"),
        ("12:30", "14:00"),
    ...
    print(f"✓ Populated {len(slots)} time slots")

def populate_course_sections(conn):
    """Create sections for the current semester"""
    ...
    if not active_sem_row:
        print("Error: No active semester found")
        return
    active_semester_id = active_sem_row[0]
    
    # Get all courses
    cur.execute("SELECT id, code, specialization_id, course_type FROM courses")
    courses = cur.fetchall()
    
    # Get teachers mapped by specialization
    cur.execute("SELECT id, specialization_id FROM teachers")
    teachers = cur.fetchall()
    teachers_by_spec = {}
    for tid, spec_id in teachers:
        if spec_id not in teachers_by_spec:
            teachers_by_spec[spec_id] = []
        teachers_by_spec[spec_id].append(tid)
        
    # Get classrooms mapped by type
    cur.execute("SELECT id, room_type_id, capacity FROM classrooms")
    classrooms = cur.fetchall()
    classrooms_by_type = {}
    for cid, rtype, cap in classrooms:
        if rtype not in classrooms_by_type:
            classrooms_by_type[rtype] = []
        classrooms_by_type[rtype].append((cid, cap))
    ...
    sections = []
    used_teachers = set() # (teacher_id, time_slot_id)
    used_rooms = set()    # (room_id, time_slot_id)
    
    # Shuffle courses to distribute sections more evenly across time slots
    random.shuffle(courses)
    
    for course_id, code, spec_id, course_type in courses:
        # Determine number of sections based on course type
        num_sections = 2 if course_type == 'core' else 1
        
        room_type_map = {
            1: 1, 2: 1, 3: 2, 4: 1, 5: 3, 6: 6, 7: 4, 8: 5, 9: 1
        }
        target_room_type = room_type_map.get(spec_id, 1)
        
        available_teachers = teachers_by_spec.get(spec_id, [])
        available_rooms = classrooms_by_type.get(target_room_type, [])
        
        if not available_teachers:
            print(f"Warning: No teachers for spec {spec_id} (Course {code})")
            continue
            
        if not available_rooms:
            # Fallback to standard room if specific not available
            available_rooms = classrooms_by_type.get(1, [])
            if not available_rooms:
                print(f"Warning: No rooms for Course {code}")
                continue

        for _ in range(num_sections):
            # Try to find an available combination
            found = False
            # Shuffle time slots to avoid concentration
            shuffled_time_slots = list(time_slots)
            random.shuffle(shuffled_time_slots)
            
            for time_slot_id in shuffled_time_slots:
                # Try teachers
                random.shuffle(available_teachers)
                for teacher_id in available_teachers:
                    if (teacher_id, time_slot_id) in used_teachers:
                        continue
                    
                    # Try rooms
                    random.shuffle(available_rooms)
                    for room_id, room_cap in available_rooms:
                        if (room_id, time_slot_id) in used_rooms:
                            continue
                            
                        # Found a valid combination
                        used_teachers.add((teacher_id, time_slot_id))
                        used_rooms.add((room_id, time_slot_id))
                        
                        section_cap = min(room_cap, 30)
                        sections.append((
                            course_id, teacher_id, room_id, active_semester_id, time_slot_id, section_cap
                        ))
                        found = True
                        break
            ...
            if not found:
                print(f"Warning: Could not find space for section of {code}")

    conn.executemany(
        """INSERT INTO course_sections 
           (course_id, teacher_id, classroom_id, semester_id, time_slot_id, capacity)
           VALUES (?, ?, ?, ?, ?, ?)""",
        sections
    )
    print(f"✓ Generated {len(sections)} course sections for the active semester")
